...
定义`Slide`对象后，我们就可以继续提取tiles了。为了加快提取过程，`histolab`会自动检测连接区域最大的组织区域，
并裁剪该区域内的tiles。`tiler`模块实现了不同的tiles提取策略，并提供了一个直观的界面，
可以轻松检索适合我们任务的tiles数据集。特别是，每种提取方法都可以通过几个通用参数进行定制：
...
-   `check_tissue`: if a minimum percentage of tissue is required to
    save the tiles;如果需要最低百分比的组织来保存瓷砖
-  `tissue_percent`: number between 0.0 and 100.0 representing the
    ...
    (default is 80.0);介于 0.0 和 100.0 之间的数字，表示组织在图像总面积中所需的最小百分比（默认值为 80.0）;
-   `prefix`: a prefix to be added at the beginning of the tiles’
    filename (default is the empty string);要在图块文件名的开头添加前缀（默认为空字符串）;
-   `suffix`: a suffix to be added to the end of the tiles’ filename
    (default is `.png`).要添加到磁贴文件名末尾的后缀（默认为“.png”）。
...
extractor:
我们可以采用的最简单的方法是从slides中随机裁剪固定数量的tiles;在这种情况下，我们需要`andomTiler`提取器：
...
假设我们想从前列腺载玻片中随机提取 30 个大小为 128 的 2 级tiles，并且只有当它们内部至少有 80% 的组织时，
我们才想保存它们。然后，我们初始化我们的`RandomTiler`提取器，如下所示：
...
reproducibility of the extraction process.请注意，我们还指定了随机种子，以确保提取过程的可重复性。
...
the ``locate_tiles`` method of ``RandomTiler`` returns a scaled version of the slide with the corresponding tiles outlined. 
It is also possible to specify the transparency of the background slide, and the color used for the border of each tile:
在开始提取过程并保存它们之前，我们可能想检查瓷砖机选择了哪些瓷砖;`RandomTiler`的`locate_tiles`方法返回slide的缩放版本，其中概述了相应的tiles。
还可以指定背景tile的透明度，以及用于每个tile边框的颜色：
...
slide as parameter:然后，开始提取就像在提取器上调用`extract`方法一样简单，将slide作为参数传递：
...
Random tiles extracted from the prostate slide at level 2.从第 2 级前列腺玻片中提取的随机tile。
...
structure on the largest tissue region detected in the WSI:我们可能希望检索所有可用的tiles，
而不是随机选择tiles。Grid Tiler 提取器在 WSI 中检测到的最大组织区域上按照网格结构裁剪瓷砖：
...
`pixel_overlap`, is set to zero:在我们的示例中，我们希望从卵巢载玻片中提取大小为 512 的
0 级方形tile，与检测到的组织数量无关。默认情况下，tiles不会重叠，即定义两个相邻tiles之间重叠像素数
的参数`pixel_overlap`设置为零：
...
同样，我们可以利用`locate_tiles`方法在幻灯片的缩放版本上可视化选定的磁贴：
