...
### Usage: Pre-Training Vision Transformers for histopathology images 用途：为组织病理学图像进行视觉转换的预训练

It is recommended that you use **CTransPath** as the preferred histopathology images feature extractor 建议使用 **CTransPath** 作为首选的组织病理学图像特征提取器

#### 1.CTransPath

##### Usage: Preparation
Install the modified [timm](https://drive.google.com/file/d/1JV7aj9rKqGedXY1TdDfi3dP07022hcgZ/view?usp=sharing) library 安装修改后的 timm 库
```
pip install timm-0.5.4.tar
```

The pre-trained models can be [downloaded](https://drive.google.com/file/d/1DoDx_70_TLj98gTf6YTXnu4tFhsFocDX/view?usp=sharing) 可以下载预训练的模型

##### Usage: Get frozen features 用法：获取冻结的特征

```
python get_features_CTransPath.py
```
It is recommended to first try to extract features at 1.0mpp, and then try other magnifications 建议首先尝试以1.0mpp的分辨率提取特征，然后尝试其他放大倍率

##### Usage: Linear Classification 用法：线性分类。
For linear classification on frozen features/weights 用于对冻结的特征/权重进行线性分类

```
python ctrans_lincls.py
```
##### Usage: End-to-End Fine-tuning 端到端微调

Similar to Swin or ViT,please see the [instructions](https://github.com/microsoft/Swin-Transformer#swin-transformer) or [DEiT](https://github.com/facebookresearch/deit) 类似于Swin或ViT，请查看说明或DEiT
