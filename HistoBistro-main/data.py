...
def get_cohort_df(
    ...
    # remove columns not in target_labels移除列
    ...
    for target in target_labels:
        df = df.dropna(subset=target) #删除NaN
        df[target] = df[target].map(lambda p: int(p) if p.isdigit() else label_dict[p])#根据字典映射
    ...
    def __getitem__(self, item):
        ...
        if self.clini_info:
            for info in self.clini_info.keys():
                clini_info = torch.Tensor([self.data[info][self.indices[item]]]
