...
# 一些全部变量
model_path = r'../output/CNN_checkpoint_best.pth'
batch_size = 256
...
# 读取数据
lib_train = torch.load(r'../output/lib/cnn_train_data_lib.db')
lib_val = torch.load(r'../output/lib/cnn_val_data_lib.db')
...
torch.save(lib_train, r'../output/lib/rnn_train_data_lib.db')
...
for i, slide_name in enumerate(lib_val['slides']):
    ...
    print('time for slide {} is {}'.format(i, time() - t0))
torch.save(lib_val, r'../output/lib/rnn_val_data_lib.db')