...
class SlideTileDataset(Dataset):
    def __init__(
        ...
        self.tiles = list(slide_dir.glob("*.jpg")) #路径的列表
        assert self.tiles, f"no tiles found in {slide_dir}"
        self.tiles *= repetitions #复制tile块
        self.transform = transform

    def __len__(self):
        return len(self.tiles) #长度为tile块的个数
...
def _get_coords(filename) -> Optional[np.ndarray]: #从文件名提取坐标并返回
    ...
    *,
    ...
    for slide_tile_path in tqdm(slide_tile_paths):
        ...
        with h5py.File(h5outpath, "w") as f:
            ...
            f["feats"] = torch.cat(feats).cpu().numpy()
